...
def create_user(username, password, nickname, role_name, is_agent):
    with app.app_context():
        ...
        db.session.add(user)
        db.session.commit()
        print(f"OK: 账号创建成功 - {username}（角色：{role_name}，代理：{'是' if int(is_agent) else '否'}）")
        return True
...
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == '--list-roles':
        ...
        if not create_user(username, password, nickname, role_name, is_agent):
            sys.exit(1)
    else:
        print("USAGE: python create_user.py --list-roles")
        print("       python create_user.py <username> <password> <role_name> <is_agent> [nickname]")
        sys.exit(1)
